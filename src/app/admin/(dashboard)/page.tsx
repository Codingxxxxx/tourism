...
export default async  function Page() {
  const dashboardState = await handleServerAction<DashboardStatistic>(getDashboardStatistic);
    ...
    <Box className='p-5' sx={{ display: 'flex', gap: 3 }}>
      <Box className='border border-slate-300 shadow rounded bg-primary' sx={{ display: 'inline-flex', alignItems: 'center', gap: 2, padding: 4 }}>
        ...
        <Box>
          ...
          <Typography>{dashboardState?.data?.listing.total ?? 'N/A'}</Typography>
        </Box>
      </Box>
      <Box className='border border-slate-300 shadow rounded bg-primary' sx={{ display: 'inline-flex', alignItems: 'center', gap: 2, padding: 4 }}>
        ...
        <Box>
          ...
          <Typography>{dashboardState?.data?.category.total ?? 'N/A'}</Typography>
        </Box>
      </Box>
    </Box>
  ) 
